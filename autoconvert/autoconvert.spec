...
AutoConvert 是一个汉字编码自动识别转换的程序，目前他可以自动识别 GB/Big5/HZ 编码，可以转换 gb/big5/hz/uni/utf7/utf8 编码。当文本超过 1000 字时，对 GB/Big5 的识别准确率接近 100%。HZ 识别率也近似 100%。这个程序最初的目的，也是它现在最大用途就是配合 procmail 对电子邮件进行汉字编码自动转换。AutoConvert 现在还很简单，不能处理 GB/Big5/HZ 混合编码。
...
install -d -m 644 $RPM_BUILD_ROOT%{_libdir}
#install -m 644 lib/*.so* $RPM_BUILD_ROOT%{_libdir}
install -D -m 755 lib/libhz.so.0.0 $RPM_BUILD_ROOT%{_libdir}/libhz.so.0.0
install -D -m 755 lib/libhz.a $RPM_BUILD_ROOT%{_libdir}/libhz.a
install -d -m 644 $RPM_BUILD_ROOT%{_datadir}
...
install -d -m 644 $RPM_BUILD_ROOT%{_includedir}
